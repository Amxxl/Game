...
namespace Dcb
{
	...
	bool ElementRefConst::Exists() const noexcept
	{
		return pLayout->Exists();
	}
	ElementRefConst ElementRefConst::operator[]( const std::string& key ) const noxnd
	{
		return { &(*pLayout)[key],pBytes,offset };
	}
	ElementRefConst ElementRefConst::operator[]( size_t index ) const noxnd
	{
		const auto indexingData = pLayout->CalculateIndexingOffset( offset,index );
		return { indexingData.second,pBytes,indexingData.first };
	}
	ElementRefConst::Ptr ElementRefConst::operator&() const noxnd
	{
		return Ptr{ this };
	}
	ElementRefConst::ElementRefConst( const LayoutElement* pLayout,const char* pBytes,size_t offset ) noexcept
		:
		offset( offset ),
		pLayout( pLayout ),
		pBytes( pBytes )
	{}
	ElementRefConst::Ptr::Ptr( const ElementRefConst* ref ) noexcept : ref( ref )
	{}
	

	ElementRef::operator ElementRefConst() const noexcept
	{
	...
	Buffer::Buffer( RawLayout&& lay ) noxnd
		:
		Buffer( LayoutCache::Resolve( std::move( lay ) ) )
	{}
	...
	ElementRefConst Buffer::operator[]( const std::string& key ) const noxnd
	{
