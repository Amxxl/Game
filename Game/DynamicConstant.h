...
namespace Dcb
{
	...
	public:
		...
		private:
			Ptr( const ElementRefConst* ref ) noexcept;
			const ElementRefConst* ref;
		};
	public:
		...
		ElementRefConst operator[]( const std::string& key ) const noxnd;
		// index into the current element as an array
		ElementRefConst operator[]( size_t index ) const noxnd;
	...
	private:
		...
		ElementRefConst( const LayoutElement* pLayout,const char* pBytes,size_t offset ) noexcept;
	...
	// version of ElementRefConst that also allows writing to the bytes of Buffer
	// see above in ElementRefConst for detailed description
	class ElementRef
	{
		friend class Buffer;
	public:
	...
	public:
		operator ElementRefConst() const noexcept;
		bool Exists() const noexcept;
